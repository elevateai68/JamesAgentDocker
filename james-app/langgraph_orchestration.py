...
james_model = ChatOllama(
    model="qwen2.5:7b", temperature=0.7, base_url="http://ollama:11434"
)
scout_model = ChatOllama(
    model="llama3.2:3b", temperature=0.3, base_url="http://ollama:11434"
)
trained_model = ChatOllama(
    model="qwen2.5:7b", temperature=0.8, base_url="http://ollama:11434"
)
...
User: {user_message}
"""
    response = james_model.invoke(
        [SystemMessage(content=james_system), HumanMessage(content=user_message)]
    )
    ...
    if "ROUTE_TO_SCOUT" in response_content:
        ...
        state["messages"].append(
            AIMessage(content=f"James routing to Scout: {scout_request}")
        )
        ...
        state["messages"].append(
            AIMessage(content=f"James routing to Trained: {trained_request}")
        )
    else:
...
def scout_agent(state: AgentState) -> AgentState:
    routing_message = next(
        (
            m.content.replace("James routing to Scout:", "").strip()
            for m in reversed(state["messages"])
            if isinstance(m, AIMessage) and "James routing to Scout:" in m.content
        ),
        "",
    )
...
"""
    response = scout_model.invoke(
        [SystemMessage(content=scout_system), HumanMessage(content=routing_message)]
    )
...
def trained_agent(state: AgentState) -> AgentState:
    routing_message = next(
        (
            m.content.replace("James routing to Trained:", "").strip()
            for m in reversed(state["messages"])
            if isinstance(m, AIMessage) and "James routing to Trained:" in m.content
        ),
        "",
    )
    if not routing_message:
...
Request: {routing_message}
"""
    response = trained_model.invoke(
        [SystemMessage(content=trained_system), HumanMessage(content=routing_message)]
    )
...
def james_final_response(state: AgentState) -> AgentState:
    ...
    history = "\n".join(
        f"{'User' if isinstance(m, HumanMessage) else 'System'}: {m.content}"
...
"""
    response = james_model.invoke(
        [SystemMessage(content=james_final_system), HumanMessage(content=user_message)]
    )
...
def create_james_workflow():
    ...
    workflow.add_conditional_edges(
        "james_coordinator",
        route_decision,
        {"scout": "scout", "trained": "trained", "james_final": "james_final"},
    )
